...
- Gates every worktree removal on three fresh checks (clean by status codes, HEAD on a durable ref, no live writer) and every branch deletion on a proven landing
- Treats `git branch -d` verdicts as noise: `-d` tests the branch's configured upstream (or HEAD), which need not be your mainline, so the landing is proven first and the ref deleted pinned to the verified sha
