...
- **Retiring the branch everywhere requires a verified LANDING**, then `git branch -D` plus `git push <remote> --delete <topic>` for the server-side ref. Before that remote deletion, confirm the remote tip still equals the tip you verified (fetch the topic and compare with `FETCH_HEAD`, as in the local-only case): a remote that moved ahead holds unverified work. Verify the landing like this:
