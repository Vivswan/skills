...
Distilled from [Wikipedia:Signs of AI writing](https://en.wikipedia.org/wiki/Wikipedia:Signs_of_AI_writing) (WP:AISIGNS). Snapshot synced 2026-07-13 from page revision 1363969535 (2026-07-13T16:57:33Z). The live page is the source of truth; refresh per SKILL.md when it changes.
...
LLMs puff up arbitrary aspects of a subject as representing or contributing to some broader topic ("marking a pivotal moment in the evolution of regional statistics"). They situate subjects amid vague "debates" and "discussions" the subject has "prompted" or "shaped", and attach importance-claims to even mundane material such as etymology or population data. For biology topics they belabor ecosystem connections and conservation status even when unknown.
...
Empirically overused words (each backed by published studies): *additionally* (especially sentence-initial), *align with*, *boasts* (= has), *bolstered*, *crucial*, *delve*, *emphasizing*, *enduring*, *enhance*, *fostering*, *garner*, *highlight* (verb), *interplay*, *intricate/intricacies*, *key* (adjective), *landscape* (abstract), *meticulous(ly)*, *pivotal*, *robust*, *showcase*, *tapestry* (abstract), *testament*, *underscore* (verb), *valuable*, *vibrant*.
...
Watch for: *serves as/stands as/marks/represents [a]*, *boasts/features/ maintains/offers [a]*, *refers to*, *holds the distinction of being*, *ventured into politics as a candidate* (= was a candidate), *began his career as* (= was).
...
Repetition penalties make models rotate through synonyms for the same referent ("the constraints of socialist realism", then "the challenging climate of Soviet artistic constraints", then "state-imposed artistic norms"), and the referent blurs with each rotation. (Caveat: some non-native English writers are taught to avoid repetition too.)

Instead: Call the same thing by the same name every time. Repetition of the right word is clarity, not a defect.
...
The Wikipedia page flags overuse: LLM text uses em dashes (—) more often than comparable human text, in places humans use commas, parentheses, or colons, in punchy sales rhythm ("we do seem to have different interpretations — and that's the problem"), and usually spaced, contrary to typographic practice.
...
A horizontal rule (`---` / `----`) before each heading, a Markdown-output habit. Instead: Headings separate sections on their own.
...
Instead: These are always bugs. Grep output for them (`turn0`, `oaicite`, `utm_source=`, `【`, `[cite:`, `:::`, `grok_`) and strip trackers off URLs.
