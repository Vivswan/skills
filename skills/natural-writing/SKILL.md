...
Produce prose that avoids the documented signs of AI writing, based on [Wikipedia:Signs of AI writing](https://en.wikipedia.org/wiki/Wikipedia:Signs_of_AI_writing). That page is a field guide for *detecting* AI text; this skill inverts it into rules for *not producing* those patterns. A single occurrence of one pattern can be legitimate; the goal is prose where the patterns don't cluster and nothing reads generated.
...
2. Snapshot: [references/signs-catalog.md](references/signs-catalog.md) is a distilled copy of the full page, reorganized as writing rules. Use it as the working checklist for every invocation; it is sufficient when offline or for routine drafting.
...
- Rewrite: revising existing text. Preserve the author's facts, structure where sound, and voice; strip only the tells. Do not inject new claims.
...
Before delivering, reread the output against the checklist above and scan it against the term lists in `references/words-to-avoid.md` (they are organized for reading, not for `grep -f`). Two checks are mechanical; run them on the draft file:
...
Fix by rewriting the sentence, not by thesaurus-swapping the flagged word: the underlying pattern (vagueness, puffery, formula) is the problem, not the word itself.
