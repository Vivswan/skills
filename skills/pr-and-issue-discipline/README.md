...
- **Release-please reads the body**: in a release-please repository, a merge that must carry Conventional Commit footers closes the `Technical details` section with a `BEGIN_COMMIT_OVERRIDE` block, which release-please reads in place of the squash message; several breaks share one multi-line `BREAKING CHANGE` footer, since release-please keeps one note per commit; the block changes release-please's parsed message, not the git commit, so a repository whose release tool reads the squash commit itself keeps its footers there
