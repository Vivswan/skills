...
- **Release tooling reads the body**: a merge that must carry Conventional Commit footers closes the `Technical details` section with a `BEGIN_COMMIT_OVERRIDE` block, which release-please reads in place of the squash message; several breaks share one multi-line `BREAKING CHANGE` footer, since release-please keeps one note per commit
