...
**Redact captured output before publishing, and publish no PII anywhere.** Strip secrets, tokens, and credentials. Replace every piece of personally identifying information with an example name: the author's employer name, real GitHub or `gh` account logins, real usernames, hostnames and machine names (`<Name>s-MacBook-Pro`), home paths (`/home/<user>`, `/Users/<name>`), and emails. One substitute per kind: `octocat`, `work-bot`, or `example-user` for a login or username; `example.com` for an employer name, a domain, or a hostname; `example-user@example.com` for a whole email, never just its host; `/home/user` or `~` for a home path; `/repo/...` for the checkout path (a captured row published with `/repo/...` in place of the machine's real checkout path is the worked example). The rule covers PR titles, bodies, commit messages, code, test fixtures, docs, captured Before/After blocks, review threads, and issue replies. Specimen: a `gh auth status` login of the form `<login>_<employer>` copied from pasted terminal output into a test fixture and a PR body; it ships as `work-bot`. A product file name that happens to contain a vendor's word (a PowerShell profile filename, a devcontainer base image) and a repository's own `owner/repo` coordinate in its install command are contracts, not PII, and stay. Redaction is not paraphrase: the command and the output structure stay verbatim.
