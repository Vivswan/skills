...
for attempt in 1 2 3 4 5; do
  ...
  run_lines="$(gh run list --commit "$sha" --json databaseId,workflowDatabaseId,workflowName --jq '.[] | "\(.databaseId)\t=\(.workflowDatabaseId)\t\(.workflowName)"' || true)"
  [ -n "$run_lines" ] && break
...
# run would report a green pipeline as red. Keep only the newest run per
# workflow id (run ids are monotonic, hence the numeric sort); older runs of
# the same workflow are informational only and never affect the exit code.
...
while IFS=$'\t' read -r id wfid name; do
  [ -n "$id" ] || continue
  wfid="${wfid#=}"
  # Ruleset/unnamed runs can lack a workflow id (jq renders it "null"); judge
  # each individually rather than collapsing them into one bogus group.
  case "$wfid" in
    "" | null)
      run_ids="$run_ids $id"
      continue
...
for id in $run_ids; do
  ...
  case "$conclusion" in
    ...
    "" | null)
      ...
      ;;
    *)
      # This includes a cancelled LATEST run: with no newer run to supersede
      # it, cancellation means the pipeline never delivered a verdict.
      fail=1
