...
- Ask the reviewer to look for:
  ...
  - The standing test question: for each NEW test in the change, the fact it pins that the source does not already say. A test that restates the source it reads (a workflow test asserting `needs` equals the list in the yaml) is a finding whose fix is deletion, and the reviewer never asks for a test without naming that fact. The template carries the wording; the `/code-standards` skill's `references/tests.md` owns the rule and its three valid answers.
