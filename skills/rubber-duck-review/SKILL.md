...
- Ask the reviewer to look for:
  - Correctness issues
  - Demonstrable defects only: a concrete input or state and the wrong output, crash, or data loss it produces in this change; a finding that cannot name the input or state is out of scope.
  ...
  - Speculative hardening (hostile callers, races in single-user tools, deadlines already bounded by an outer timeout, "consider validating") is listed under a `Recorded, not built` heading, never as a finding; the exception is a repository with more than 100 GitHub stars, where such items are surfaced to the user and built only after they confirm.
...
- Treat valid "non-blocking" feedback as real work when it is a correctness or maintainability finding with a demonstrated effect in this change. Items under `Recorded, not built` are not work: leave them in the report and open no task or PR for them. The one exception is a repository with more than 100 GitHub stars: surface those items to the user with the concrete risk, and build only the ones they confirm.
