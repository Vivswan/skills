...
- PII anywhere in the diff, the PR body, or the commit messages: an employer name, a real GitHub or gh account login, a real username, a hostname or machine name, a home path under a real user (`/home/<user>`, `/Users/<name>`), an email. Example names only (`octocat`, `work-bot`, `example-user`, `example.com`, `/home/user`). A hit is blocking; quote the string and where it sits. A product file name that contains a vendor's word and a repository's own `owner/repo` coordinate in its install command are contracts, not PII.
