...
- Picks the highest-rung response per shape (rung 1 wherever it exists): a generated single source over convention-synced duplicates (a rung-2 CI agreement check where generation is impractical), closed unions with exhaustiveness over string dispatch, branded types validated once at the boundary over re-validated identifiers, rung-2 executable checks over trap docs, sets and richer state over counts and booleans, persisted stores over evaporating state
