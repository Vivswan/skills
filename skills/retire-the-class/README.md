...
- Picks the class-retiring form: a generated single source over convention-synced duplicates (a CI agreement check where generation is impractical), closed unions with exhaustiveness over string dispatch, branded types validated once at the boundary over re-validated identifiers, executable checks over trap docs, sets and richer state over counts and booleans, persisted stores over evaporating state
