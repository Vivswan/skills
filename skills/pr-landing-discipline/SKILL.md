...
{ gh pr view "$n" --json title,body,commits -q '.title, .body, (.commits[] | .messageHeadline, .messageBody)' &&
  ...
  { grep -n -i -F -e 'ExampleCorp' -e 'real-login' -e 'MacBook-Pro' -e '/Users/realuser' "$out"; echo "grep exit $?"; }
rm -f "$out"
```
