...
- **Babysit to comment convergence**: every review round triaged the same cycle it appears, valid findings fixed in that round, invalid ones replied to and resolved, bot comments advisory and filtered to demonstrable defects (speculative hardening and unenforced style declined in one sentence, the bot's summary verdict not a gate), thread state read via GraphQL `isResolved`, recurring finding classes swept in one pass instead of one instance per round
...
- **Who merges**: the human by default, two narrow standing exceptions, the owner's `merge-when-green` label (a hand-over of the merge for the PR's whole life, not a verdict on the content, so the operator runs every gate on the exact head that lands), and an exit-conditioned landing that never chains the merge behind reading a gate log
