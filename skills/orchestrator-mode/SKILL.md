...
- **Line accounting before every landing** is defined in the `/pr-landing-discipline` skill: the per-kind read of the frozen diff against the stated purpose, the purpose-to-shape table, the sums placed in the PR body by whether they fit the purpose. The lead runs it at the landing gate alongside the review, in direct mode too (the sums go in the landing report instead of a PR body), and a mismatch routes back to the owning builder per Review Loops (4). The fleet adds the wave-level read, since growth accumulates one reasonable PR at a time: after a wave lands, and retroactively over a wave landed before the rule was in force, pull each landing's rows (the skill's `pulls/<n>/files` form per PR; `git diff --numstat <sha>^..<sha>` per direct commit), read every landing's sums against its own purpose and the wave's total against the wave's, and report the ones that fail.
