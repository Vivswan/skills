...
- **Local test runs are targeted, at every loop.** Builders, gates, and the lead run only the tests the change and its proof touch; the full suite and the lint-plus-build sweep are the PR's CI's, never run locally when CI runs them anyway. Item 3 of `references/spawn-briefs.md` carries the rule every brief states.
