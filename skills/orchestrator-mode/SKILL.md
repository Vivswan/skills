...
1. **Landing mode**: direct commits to main, direct commits to a designated branch (which one?), a PR per track, or a stacked-PR chain? With PRs, also settle WHO MERGES: the user by default - a PR exists to put a human gate before the mainline, so the lead only prepares each PR (push, gates, a "ready to merge" report) and the user merges; the lead merges only on the user's explicit delegation, or when a merge queue owns the ordering. A user delegating merges wholesale should be offered direct commits instead: lead-merged PRs are direct commits with extra steps, worth keeping only where branch protection or a merge queue forces the PR mechanism. Dependent tracks (each building on the previous) favor the stack; independent tracks favor flat PRs. Infer the likely answer from branch protection, CONTRIBUTING docs, and recent history; when unsure, propose a PR per track (the safer guess on a protected repo).
...
4. In a PR landing mode, read `references/landing.md` BEFORE fanning out: stacked PRs initialize the chain (`gh stack init`) before builders spawn, and PR-per-track briefs must name who pushes and opens each draft.
...
Implementation parallelizes; integration into a shared base does not. "The mainline" is whatever branch the session integrates changes into - often main, but the user may designate any branch, and every flow applies unchanged. The per-mode procedures for the three landing modes the interview chose between - serial landings (direct-push repos), stacked PRs (one dependent chain, including the gh-stack loop and its worktree interplay), a PR per track - live in `references/landing.md`. What stays constant in every mode:
...
When the host agent cannot spawn subagents, keep the structure and drop the parallelism: plain `git worktree add` for isolation, tracks worked one at a time in dependency order, background read-only CLI invocations of another agent (`codex exec`, `claude -p`) in place of worktree builders where available - and every gate kept: review before landing, serialized integration, CI watch after every push or merge, the post-landing integration review.
...
- `references/landing.md`: the three landing-mode procedures (serial direct-push, stacked PRs with the gh-stack loop, a PR per track)
