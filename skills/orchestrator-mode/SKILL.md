...
1. **Landing mode**: the GATE, chosen once. Direct commits to the mainline (which branch?), or a PR before it?
   - With PRs, also settle WHO MERGES. The default (the user) and the two standing exceptions are defined in the `/pr-landing-discipline` skill; the lead merges only on the user's explicit delegation, when a merge queue owns the ordering, or under those exceptions.
...
3. **Spawn the fleet monitor first**: one subagent whose only job is watching the others and messaging the lead when any agent stalls or dies. The rule is per-FLEET, not per-session: when a monitor has stood down and a new wave of workers spawns later in the same session, a fresh monitor spawns FIRST, same as at session start, pointed at the prior monitor's ledger for continuity.
   ...
   - The brief names the session's PII needle file (`/tmp/fleet-<sessionId>/pii-needles.txt`, written by the lead at session start, outside the repository): on every sweep where a worktree's `headSha` moved, the monitor greps that track's added lines and commit messages with it and flags a hit to the lead. Wiring in `references/fleet-monitor.md`, PII on Every HEAD Move.
...
- **Each worktree agent runs its own review loop** before reporting back. The lead's landing-gate review then gates the LANDING on the change's final integrated state (the rebased branch, or the up-to-date PR): an integration review, not a first look. The two loops are defense in depth, not repetition: each catches failures the other is blind to, and skipping a gate requires naming which remaining gate covers its failure class; "the other one passed" is not an answer. Where the `/rubber-duck-review` skill is installed, use it for both loops: a cross-model, read-only reviewer.
- **Local test runs are targeted, at every loop.** Builders, gates, and the lead run only the tests the change and its proof touch; the full suite and the lint-plus-build sweep are CI's, on the push or the PR, never run locally when CI runs them anyway. Item 3 of `references/spawn-briefs.md` carries the rule every brief states.
  - About three gates run at once, and every long run carries an explicit timeout.
  - A personal private repository meters CI minutes, so there the full check runs locally before the push instead; a lead who is not sure which kind the repository is asks the user.
...
- **Before a builder writes a static-analysis guard (an AST walk, a reach analysis, a fail-closed scanner), the brief states the guarantee's SCOPE in one sentence.** Production: a fail-closed child-spawn guard took five landing-gate rounds, each round finding another way to hide a spawn, until the guarantee was narrowed to "accidental omissions and analysis gaps; deliberate hiding is out of scope". Without that sentence every review round widens the target, and builder and reviewer argue over a guarantee nobody wrote down.
...
- **Who merges (the human by default), the two standing exceptions and the owner's `merge-when-green` label, and the exit-conditioned landing** are defined in the `/pr-landing-discipline` skill, show-the-change PR bodies with redaction in the `/pr-and-issue-discipline` skill. In the PR gate all of them apply; in direct mode there is no PR body or merging hand, so what carries over is the exit-conditioned landing.
- **Review before landing, never after**: findings must be able to block the landing. The review scopes to the exact content being landed, frozen until it lands: the branch or PR diff (`base...HEAD`) once the work is committed, the staged diff when landing a still-uncommitted patch, never just "the working tree", whose clean state at merge time would make the review vacuous. Where installed, the `/rubber-duck-review` skill defines the review and its convergence; nothing lands until it has converged.
- **The PII gate before every landing**, in the PR gate and in direct mode alike: the `/pr-landing-discipline` skill's READY grep, run with the session's needle file over every commit's own added lines and message (never only the net diff: a fixture added in one commit and deleted in the next is still published by the first), the whole replacement fixture, and (with a PR) the title, body, and every comment. In direct mode the read is the landing-gate variant in `references/fleet-monitor.md`, PII on Every HEAD Move: every commit's message and added lines since the branch point plus the whole replacement fixture from the frozen tree, or, for a patch still uncommitted, the staged fixture, the staged diff's added lines, and the proposed message. The needle file carries the owner's real identifiers and, when the change replaces a fixture measured from real data, every full-precision ratio and 4+ digit integer of the old file. A hit routes back to the owning builder per Review Loops (4); a fixture with a provenance comment naming real data, or a quoted real configuration or log, is a finding whose fix is a hand-written substitute. The rule itself (nothing published tells a reader who the owner is, how they work, or how their machine is set up) and the substitutes are the `/pr-and-issue-discipline` skill's.
- **Line accounting before every landing** is defined in the `/pr-landing-discipline` skill: the per-kind read of the frozen diff against the stated purpose, the purpose-to-shape table, the sums placed in the PR body by whether they fit the purpose. The lead runs it at the landing gate alongside the review, in direct mode too (the sums go in the landing report instead of a PR body), and a mismatch routes back to the owning builder per Review Loops (4) for the trim or the reason; a good change that grew still lands, with the reason where the user sees it. The fleet adds the wave-level read, since growth accumulates one reasonable PR at a time: after a wave lands, and retroactively over a wave landed before the rule was in force, pull each landing's rows (the skill's `pulls/<n>/files` form per PR; `git diff --numstat <sha>^..<sha>` per direct commit), read every landing's sums against its own purpose and the wave's total against the wave's, and report the mismatches with their reasons.
- **After every push or merge** (a direct landing, a PR update, or a PR merge), spawn a background CI watcher that reports pass/fail with failing-job logs. Never fire-and-forget a push, and never watch CI inline. After a MERGE, the SHA to watch is the MAINLINE tip, never `git rev-parse HEAD` in the worktree: HEAD is still the topic branch's tip, and a squash or merge commit is a new SHA that exists only on the mainline (`git fetch <base-remote> <mainline> && git rev-parse FETCH_HEAD` names it, fetching from the remote the PR merges into, never a fork remote). Where installed, the `/watch-ci-after-push` skill defines the watcher (run discovery, the full-SHA gotcha, the merge recipe, the report format). One CI watcher per session at a time, its sustained watch interval at least 60 s: queue the next push behind the running watcher, or, when the mainline's runs share a concurrency group key, watch only the newest mainline tip (a newer push cancels an older tip's pending run, so that watcher can end with no verdict). Production: parallel watchers at gh's default 3 s refresh drained GitHub's 5000-requests-per-hour REST bucket and blinded every CI verdict for 45 minutes.
