...
- **A valid finding is fixed by the OWNING builder**, routed back to amend in place; the lead never patches in a builder's tree.
...
- **Who merges (the human by default), the two standing exceptions, visualization-first PR bodies with redaction, and the exit-conditioned landing** are defined in the `/pr-and-issue-discipline` skill and apply to every landing here, PR or direct.
