...
- **A zero `treeFileCount` on the main-checkout row makes the sweep report `control: FAILED` and exit 1; re-run it.** sweep.mts once read 0 on that control row for two consecutive sweeps with every other column consistent, then recovered: an instrument reading, never a wipe call, and never a baseline to compare worktree rows against, so the script refuses it instead of leaving the call to the reader. A zero on a WORKTREE row stays the wipe signal.
