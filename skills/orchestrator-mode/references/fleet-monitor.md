...
- `aheadBehind` is measured against a sha of origin's default branch pinned ONCE for the whole sweep, so two rows of one pass cannot disagree because a fetch moved the ref mid-sweep. Across sweeps the mainline still moves: when readings contradict, re-fetch and re-read rather than trusting the earlier number. The base is origin's DEFAULT branch only - the script takes no base-ref option - so in a session integrating into a designated non-default mainline, do not read landed-state conclusions from this field; use the baseline and probe checks in Verifying a Landing instead.
