...
One subagent whose only job is watching the rest of the fleet. It starts a periodic check (a cron or scheduled wakeup where the harness offers one, a sleep loop otherwise) and messages the lead when any agent stalls or dies.
...
What remains is the judgment layer: what the readings mean, when to flag, and how to report. Every rule below comes from a false conclusion caught in production. Do not simplify them away, and do not hand-roll shell probes around the scripts: when a reading is missing, extend the script.

The measurement rules that are not monitor-specific (evidence-or-error probes, positive and negative controls, suspecting the instrument, postconditions over logs, the look-then-send re-measure) live in the `/verify-with-controls` skill. This file keeps their monitor-specific instantiations and wiring.

In this skill's home repository, `tests/doc-drift.test.ts` pins the flags, field names, and schema strings this file cites to the script sources: a rename on either side fails that repository's CI until doc and script move together. An installed copy inherits whatever the repository verified at release.
...
```bash
bun <skill-dir>/scripts/sweep.mts <repo-root> [--base <ref>] [--transcripts <dir>]
```
...
- **The activity fingerprint is `headSha` plus `newestDirtyMtime` plus `statusHash`, all three together.** The newest mtime among dirty and untracked files catches edits. The HEAD sha catches clean amends and rebases (which move HEAD with no dirty files and an unchanged commits-ahead count). And `statusHash` (the path-and-status set) catches deletion-only work, which the other two cannot see: a deleted path cannot be stat'd, so it bumps `dirtyCount` and the hash while `newestDirtyMtime` stays frozen. Never the git index, which can sit frozen at spawn time for 12.5 measured minutes while a worker writes hundreds of lines. The hash alone is not enough either: it freezes the moment a worker stops adding files and just keeps editing the same ones, which is exactly what `newestDirtyMtime` is there to catch.
- **`aheadBehind` is measured against a base sha pinned ONCE for the whole sweep**, so two rows of one pass cannot disagree because a fetch moved the ref mid-sweep. Across sweeps the mainline still moves: when readings contradict, re-fetch and re-read rather than trusting the earlier number. The base defaults to origin's DEFAULT branch; pass `--base <mainline>` (any committish, e.g. `--base origin/develop`) for sessions targeting a non-default mainline, so the counts measure the branch the fleet actually lands on. An unresolvable or ambiguous `--base` ref is loud, never zeros and never a silent fallback to the default: the sweep emits a leading `{"baseRef":{"ok":false,...}}` line and rows carry `aheadBehind:null`.
- **`treeFileCount` (files at HEAD) is the repo-wipe detector.** HEAD sha, ahead/behind, dirty count, and mtimes all read normal for a commit stacked on top of a repo-wiping commit; only the file count separates "committed its work" from "committed on top of deleting the repo". The comparison is executable from ONE sweep output: the main checkout is always emitted as the first row (it doubles as the positive control), so compare each worktree row's `treeFileCount` against the main-checkout row's. No hand-rolled base probe needed.
...
A broken probe is loud, never zero:

- A vanished worktree or failed git reading is an `ok:false` row carrying its error.
- A row whose HEAD, branch, or gitdir moved mid-measurement refuses itself as `ok:false` rather than mixing two states (the next sweep re-measures).
- A degraded or downed instrument is a leading `{"control":"FAILED",...}`, `{"lsof":{"ok":false,...}}`, `{"defaultRef":{"ok":false,...}}`, or `{"baseRef":{"ok":false,...}}` line.
- Partial lsof visibility is its own documented state, not a failure: lsof exiting nonzero while still producing records emits `{"lsof":{"ok":true,"degraded":true,...}}`, and process lists may omit unreadable processes. Under degraded, an EMPTY `processes` list is NOT verified inactivity; treat it as unknown and escalate rather than reading absence into it.
- Exit 0 means the sweep ran, `ok:false` rows included. Exit 1 is reserved for a control failure (worktree discovery failed, or the main checkout's own row, the positive control, is impossible).
- A downed lsof, an unresolvable default branch, or an unresolvable or ambiguous `--base` ref still exits 0 with its warning line. So read the leading lines and the per-row `ok` fields, never the exit code alone. A dead sweep is an instrument failure, never a quiet fleet.

## Reading a Sweep

The row is data; these rules are what it means:

- **Two observations before any flag.** For "did anything happen": require ALL THREE of `newestDirtyMtime`, `headSha`, and `statusHash` stable across two sweeps before flagging inactivity. Deletions are invisible to mtime, so the hash is load-bearing. For "is anything happening now": judge the `processes` list by its command names and state codes across two sweeps. The sweep emits exactly `pid`, `command`, and `state` per process, nothing more.
- **Presence is not work.** A `processes` list that is a single shell row (one `zsh`/`bash` entry) parked on the worktree cwd reads as "alive" under a presence test and silently voids idle counts that should keep running. A working agent shows WORKER rows: every process whose cwd is inside the worktree is its own row, so bun/node/test-host children appear alongside the shell. The sweep row cannot answer "is this lone shell childless and idle" (it carries no child list and no CPU). That is an escalation pair, run only when a suspicious row set warrants it: `pgrep -P <shell-pid>` for children (empty output with exit 1 means none; ps's ppid column names the shell's PARENT, so it can never establish childlessness) plus `ps -o pcpu=,stat= -p <pid>` for CPU and state. Even then judge by children and state, never a single CPU sample: an I/O-bound test host reads near-zero CPU while fully busy.
...
- **A zero `treeFileCount` on the main-checkout row makes the sweep report `control: FAILED` and exit 1.** It is never a wipe call and never a baseline for worktree rows, so the script refuses it. The one instrument fault that produced it is fixed: launched from inside a nested worktree, git read the cwd as a pathspec prefix and the row's `ls-tree` listed only paths under it, so sweep.mts read 0 on a healthy repo. Every row's git calls now run from the worktree itself and `ls-tree` carries `--full-tree`, so the reading no longer depends on the launch cwd. A zero that still appears means the main checkout's HEAD really is an empty tree. A zero on a WORKTREE row stays the wipe signal.
...
- **Size a track base-to-HEAD** (its branch-point commit to its HEAD), never `diff origin/main..HEAD`: for a behind track the latter also reverses every sibling landing in the gap (observed seven-fold deletion inflation). Reserve comparisons against the sweep's selected base for the ahead/behind counts the sweep already carries.
...
Every result is evidence-bearing or a loud error, never a bare number:

- Each match carries the START line's text plus its line range. A whitespace-normalized match can span lines: `endLine` marks the span's last line, and the remaining lines are NOT attached, so open the file at that range to inspect the full span.
- Probed paths are existence-validated before any zero is trusted. JSON is parsed rather than grepped. Change sets union committed and dirty paths.
- Literal matching is fixed-string, token-bounded for ASCII word characters, and whitespace-normalized. An ASCII literal embedded in a longer word or filename does not count (so `release.yml` can never match `update-release.yml`). Every whitespace run in both literal and text collapses to a single space before matching, so a reflow that rewraps a paragraph across line boundaries cannot produce a false 0 by construction.
- Exit semantics differ by subcommand. `tokens` and two-file `json-keys` are checks: exit 0 is a verified pass, exit 1 a failed check. `count` and single-file `json-keys` and `set` are measurements: exit 0 means the measurement succeeded, and the verdict is yours to read from the value and evidence. Exit 1 is also any broken probe (a vanished file fails every token loudly; a negative token can never read a missing file as 0 occurrences and silently pass). Exit 2 is usage.
...
Saves are temp-plus-rename atomic. `init` is an atomic create-if-absent (it never clobbers an existing ledger). The mutating commands (`state`, `flag`, `retract`, `grant`) run under a lockfile; a mutation never proceeds unlocked. So duplicate flags are refused globally even under concurrent writers, dormancy is a lookup, and grants keep their exact wording for later literal-vs-intent re-reads. Without this, a lead's "do not flag X, it is parked on purpose" instruction evaporates two rounds later and the same false suspicion re-fires each sweep.

What a parked worker may still report:

- A `dormant-by-design` worker's only reportable events: its worktree vanishing, its HEAD moving (new commits appearing OR an amend/rebase, where the commits-ahead count may not change), or new dirty or untracked files appearing while parked.
- A `landing-gate` worker's branch may legitimately gain a fix commit or disappear entirely (verified landing).

Two disaster-shaped non-disasters:

- **HEAD == the measured base ref at 0-ahead/0-behind is the post-landing-swept signature only with a CLEAN tree**, and only when the session's selected mainline IS that measured ref: pass `--base <mainline>` for sessions targeting a non-default mainline, so the signature reads against the branch the session actually lands on. With a dirty tree it is the opposite, a deliberate WIP collapse mid-build (rebase the WIP commits, then reset onto the new base; a 51-file package once read as vanished this way). The discriminators are the dirty count and the reflog, and `parent..HEAD` sizing is meaningless after the reset because HEAD is no longer the track's commit.
- **A falling insertion count on a removal track is the expected direction, not lost work.** Reconcile it arithmetically against the folded-in rounds instead of flagging.
...
- **The meta-lesson:** a known-bad probe can re-enter through someone else's wording of a flag condition. Memory is no defence; measuring again immediately before firing is. A near-miss stays a near-miss only because the monitor re-checked instead of trusting its own earlier authorization. The general bar (two observations decide whether to LOOK, a fresh re-measure immediately before sending decides whether to SEND) is `/verify-with-controls` rule 5.
...
- **Standing state lives in the ledger, not in messages.** A send that reports success proves QUEUING, not delivery: a monitor mid-turn drains its inbox only at its next tool round. One production session, sketched:
  ...
  Anything the monitor must honor across sweeps goes into the ledger in the shape it represents, and the monitor re-reads it every sweep. A message is a one-shot exchange, never a standing order.

  - "Parked on purpose" is a `state` transition.
  - A scope adjudication is a `grant` with its exact wording.
  - A closed finding is a `retract`.
...
- **An unacknowledged directive is undelivered.** A directive sent as a message is in force only once acknowledged; until then the sender treats it as not delivered and re-sends it (or writes it to the ledger, where the next sweep must read it). The monitor's half: acknowledge every lead directive in its next report. Re-send only after confirming the directive is still current; a still-current re-send is safe because every directive is written to be idempotent, per `references/spawn-briefs.md`, Directives and Steps Are Idempotent, while re-sending a superseded directive restores stale intent.
- **The stand-down is the one directive the lead does not wait on.** An idle teammate's inbox drains only at its next tool round, so a stand-down message can sit unread for 20+ minutes while the lead waits for an acknowledgement that cannot come. Once the fleet is landed, the lead sends the stand-down for the record and then TaskStops the monitor directly.
...
- **A rebase can silently no-op as well as silently drop.** Before an INDEPENDENT behind track (one whose branch does not already contain the sibling's content) rebases across a sibling's landing, run the SIBLING's token table against the un-rebased tree (it must FAIL there) and again after the rebase, where it must pass: the flip proves the replay genuinely happened, independently of the ahead/behind counts. Two measurements, one conclusion. The flip is INVALID for a dependent (stacked) successor: its branch already contains the sibling's tokens before the restack, so the pre-restack probe correctly passes and the rule would report a false failure. Verify dependent successors by stack ancestry and the per-site postconditions in `references/landing.md` instead.
